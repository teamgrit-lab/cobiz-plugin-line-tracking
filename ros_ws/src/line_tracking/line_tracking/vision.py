...
class VisionConfig:
    ...
    hsv_lower: Tuple[int, int, int] = (14, 45, 40)
    hsv_upper: Tuple[int, int, int] = (42, 255, 255)
    lab_b_min: int = 135
    adaptive_lab_percentile: float = 85.0
    ...
    def validate(self) -> None:
        ...
        if self.open_kernel <= 0 or self.close_kernel <= 0:
...
class YellowLineVision:
    ...
    def _color_mask(self, frame_bgr: np.ndarray, roi_mask: np.ndarray) -> np.ndarray:
        ...
        return self._remove_small_components(mask)
    ...
    def process(self, frame_bgr: np.ndarray) -> VisionResult:
        ...
        if self._segmenter is None:
            ...
            mask = self._color_mask(frame_bgr, roi_mask)
            road_mask = None
            raw_line_mask = None
        else:
